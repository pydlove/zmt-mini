...
public interface StyleConfigMapper {
    ...
    @Insert("INSERT INTO tu_style_config(id, name, strategy, params, is_active, created_at, updated_at) " +
            "VALUES(#{id}, #{name}, #{strategy}, #{params, typeHandler=org.apache.ibatis.type.StringTypeHandler}, #{isActive}, NOW(), NOW())")
    int insert(StyleConfig config);

    @Update("UPDATE tu_style_config SET is_active = 0")
    int deactivateAll();

    @Update("UPDATE tu_style_config SET name = #{name}, strategy = #{strategy}, params = #{params, typeHandler=org.apache.ibatis.type.StringTypeHandler}, " +
